...
import { reorderEventCategories } from "@/lib/vitrix/event-categories";
...
export async function POST(req: NextRequest) {
  try {
    ...
    await reorderEventCategories(supabase, normalized);
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("POST /api/vitrix/events/categories/reorder error:", err);
    await logVitrixError(err, "/api/vitrix/events/categories/reorder");
