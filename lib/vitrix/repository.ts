...
export async function getVitrixBootstrap(_user?: VitrixUser | null): Promise<VitrixBootstrap> {
  void _user;

  if (process.env.NODE_ENV === "development") {
    const settings = await getSiteSettings().catch(() => DEFAULT_SITE_SETTINGS);
    const logs = await listVitrixLogs().catch(() => ({ open: [], resolved: [], all: [] }));
    return {
  ...
  try {
    ...
    const [modules, catalogue, press, roles, permissions, userRoles, authUsers] = await Promise.all([
      supabase
        .from("vitrix_modules")
      ...
      supabase
        ...
        .order("sort_order"),
      supabase.from("news").select("id,category,venue,title,event_date,type,lang,status"),
    ...
    if (modules.error || roles.error || permissions.error || userRoles.error || authUsers.error) {
    ...
    userRoles.data?.forEach((row) => {
      ...
      if (!role?.name) return;
      roleMap.set(row.user_id, [...(roleMap.get(row.user_id) ?? []), role.name]);
    });
